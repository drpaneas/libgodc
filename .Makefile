...
GCCGO = $(KOS_CC_BASE)/bin/sh-elf-gccgo $(GCCGO_OPTS) -I$(KOS_BASE)/lib -L$(KOS_BASE)/lib
OUTPUT ?= $(TARGET).elf
...
# -lm -lc are the math and C standard libraries (required by KOS)
LIBS = -Wl,--whole-archive -lgodcbegin -Wl,--no-whole-archive -lkos -lgodc -lparallax -lkosutils -ljpeg -lpng -lz -lkmg -lwav -ltremor -lm -lc
...
clean:
	rm -f *.o *.elf romdisk.img .Makefile go.mod
